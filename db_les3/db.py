...
class Database:
    ...
    def create_post(self, data):
        ...
        author = self._get_or_create(session, models.Author, models.Author.url, data["writer"]["url"], **data["writer"])
        post = self._get_or_create(session, models.Post, models.Post.url, data["posts"]["url"], **data["writer"])
        post.author=author
        post.tags.extend(map(
            lambda tag_data: self._get_or_create(
                session, models.Tag, models.Tag.url, tag_data["urltag"], **tag_data
            ),
            data["tags"],
        ))
